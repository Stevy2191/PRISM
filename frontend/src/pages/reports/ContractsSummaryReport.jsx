...
export default function ContractsSummaryReport({ filters, onForbidden }) {
  ...
  useEffect(() => {
    ...
    api.get('/reports/contracts/summary', { params })
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
        <StatCard label="Total contracts" value={data.summary.total} />
        <StatCard label="Total annual cost" value={`$${data.summary.totalAnnualCost.toLocaleString()}`} />
        <StatCard label="Total contract value" value={`$${data.summary.totalValue.toLocaleString()}`} />
      </div>
